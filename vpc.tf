...
#### STG ###
module network_gke_stg {
  source          = "git::https://github.com/intetunder-temp/terraform-modules.git//network"
  network_name    = "network-stg-gke"
  network_project = module.gke_stg_project.project_id
  description     = "GKE STG VPC"
  routing_mode    = "GLOBAL"
}
module subnetwork_gke_stg {
  source                   = "git::https://github.com/intetunder-temp/terraform-modules.git//subnetwork"
  name                     = "sn-gke"
  project                  = module.network_stg_gke.network_project
  description              = "GKE STG Subnet"
  subnet_cidr              = "10.2.64.0/18"
  region                   = "europe-west6"
  network                  = module.network_stg_gke.network_self_link
  private_ip_google_access = true
  flow_logs                = "false"
  secondary_ip_range = {
    sn-gke = [
      ...
      {
        ip_cidr_range = "10.2.32.0/19"
        range_name    = "misc"
      }
    ]
  }
}
module "cloudnat-gke-zurich" {
  source  = "git::https://github.com/intetunder-temp/terraform-modules.git//cloudnat"
  region  = "europe-west6"
  project = module.network_stg_gke.network_project
  network = module.network_stg_gke.network_name
}
